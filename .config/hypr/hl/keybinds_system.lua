...
hl.bind(mainMod .. " + CTRL + SHIFT + R", function()
  ...
  --  hl.notification.create({
    ...
    --})
end,    { description = "Reload Hyprland configs" })
...
{description = "mount drives with fuzzel"})
...
---------------------------------------------------------------
-- TASK MANAGER BTOP
hl.bind("CTRL + SHIFT + Escape",  hl.dsp.exec_cmd("app2unit -- kitty --class btop-float -e btop"), {description = "task manager (btop)"})
  ...
  --  hl.notification.create({
    --    text = "Renderer reloaded!",
      --  timeout = 5000, -- duration in milliseconds
        --icon = 5,       -- icon type (5 corresponds to OK/success)
    --})
end, 	{ description = "Aquamarine Renderer reload"} )
