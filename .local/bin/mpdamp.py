...
# Change process name in the process list
...
APP_VERSION = 'MPD⚡AMP 0.1-beta-v17 aka "The Little Beast"'
...
class MPDAmp(tk.Tk):
    ...
    def build(self):
        ...
        self.mute_btn.pack(side='left', padx=(7,3))
    ...
    def _sort_tree(self, tree, column, numeric=False):
        reverse = getattr(self, '_sort_reverse', {}).get(column, False)
    ...
    def _heading_sort(self, tree, column, numeric=False):
        self._sort_tree(tree, column, numeric)
    ...
    def build_queue(self):
        ...
        for c in cols:
            self.queue_tree.heading(c, text=headings[c], command=lambda c=c: self._heading_sort(self.queue_tree, c, c=='#'))
    ...
    def build_database(self):
        ...
        for c,h in zip(cols,('ARTISTS','ALBUM','SONG')):
            self.db_tree.heading(c,text=h,command=lambda c=c: self._heading_sort(self.db_tree, c))
        self.db_tree.column('artist',width=260); self.db_tree.column('album',width=220); self.db_tree.column('song',width=300)
    ...
    def build_search(self):
        ...
        for c,h in zip(cols,('ARTIST','ALBUM','SONG')):
            self.search_tree.heading(c,text=h,command=lambda c=c: self._heading_sort(self.search_tree, c))
    ...
    def on_result(self, tag, result):
        ...
        if isinstance(tag, tuple) and tag and tag[0] == 'db_album':
            self.db_rows = [{'name': x.get('album',''),
                             'artist': tag[1], 'album': x.get('album',''), 'title': ''} for x in (result or [])]
            self.render_db(); return
        if tag == 'db_album':
            self.db_rows = [{'name': x.get('album',''), 'artist': '', 'album': x.get('album',''), 'title': ''} for x in (result or [])]
            self.render_db(); return
        if tag == 'db_tracks':
            self.db_rows = [{'name': x.get('title') or os.path.basename(x.get('file','')),
                             'artist':x.get('artist',''), 'album':x.get('album',''),
                             'title':x.get('title') or os.path.basename(x.get('file','')),
                             'file':x.get('file','')}
                            for x in (result or [])]
            self.render_db(); return
        if tag == 'db':
    ...
    def on_db_result(self,result):
        ...
        for x in result or []:
            ...
            if name: self.db_rows.append({'name':name,'artist':name,'album':'','title':''})
        self.render_db()

    def render_db(self):
        for iid in self.db_tree.get_children(): self.db_tree.delete(iid)
        for i,r in enumerate(self.db_rows):
            self.db_tree.insert('', 'end', iid=f'd{i}', values=(r.get('artist','') or r.get('name',''), r.get('album',''), r.get('title','')))
    ...
    def db_add_row(self,r):
        if self.db_mode=='tracks': self.executor.submit('add',r.get('file'),tag='add'); return
        if self.db_mode=='artists': self.executor.submit('findadd','file',r.get('artist') or r.get('name',''),tag='add'); return
        if self.db_mode=='albums': self.executor.submit('searchadd','album',r.get('album') or r.get('name',''), 'artist', r.get('artist',''), tag='add')
